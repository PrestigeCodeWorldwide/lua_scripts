...
local function cwtnCHOSEN()
    if mq.TLO.CWTN.Mode() ~= State.chosenMode then
        ...
        mq.cmd("/squelch /nav stop")
		mq.delay(250)
        mq.cmdf("/%s mode %s", my_class, State.chosenMode)
        mq.delay(250)
...
local function init()
	initMQBindings()
	mq.imgui.init("OffTanking", State.OT_UI)
...
State.OT_UI = function()	
	if not State.open_gui or mq.TLO.MacroQuest.GameState() ~= "INGAME" then
		return
	end
	State.open_gui, State.should_draw_gui = ImGui.Begin("Offtank XTar", State.open_gui)
	
	
	if State.should_draw_gui then
        if pause then
		...
		if State.isChanged then
			BL.info(State.distance)
		end
