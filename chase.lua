...
local function do_chase()
    ...
    if not chase_spawn or not chase_spawn() then return end
    ...
    if check_distance(me_x, me_y, chase_x, chase_y) > DISTANCE^2 then
        if not mq.TLO.Navigation.Active() and mq.TLO.Navigation.PathExists(string.format('spawn pc =%s', chase_spawn.CleanName())) then
