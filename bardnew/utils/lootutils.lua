...
local validActions =
	{ keep = 'Keep', bank = 'Bank', sell = 'Sell', ignore = 'Ignore', destroy = 'Destroy' }
...
local function checkCursor()
	...
	while mq.TLO.Cursor() do
		...
		if mq.TLO.Me.FreeInventory() == 0 or mq.TLO.Cursor() == currentItem then
			if loot.SpamLootInfo then
				print(logger.logLine('Inventory full, item stuck on cursor'))
			end
			mq.cmd('/autoinv')
			return
		end
...
local function getRule(item)
	...
	lootData[firstLetter][itemName] = lootData[firstLetter][itemName]
		or lookupIniLootRule(firstLetter, itemName)
	if lootData[firstLetter][itemName] == 'NULL' then
		if tradeskill then
			lootDecision = 'Bank'
		end
		if sellPrice < loot.MinSellPrice then
			lootDecision = 'Ignore'
		end
		if not stackable and loot.StackableOnly then
			lootDecision = 'Ignore'
		end
		if loot.StackPlatValue > 0 and sellPrice * stackSize < loot.StackPlatValue then
			lootDecision = 'Ignore'
		end
...
local function setupEvents()
	...
	if loot.LootForage then
		mq.event(
			'ForageExtras',
			'Your forage mastery has enabled you to find something else!',
			eventForage
		)
...
local function lootCorpse(corpseID)
	...
	mq.delay(3000, function()
		return cantLootID > 0 or mq.TLO.Window('LootWnd').Open()
	end)
	if not mq.TLO.Window('LootWnd').Open() then
...
function loot.lootMobs(limit)
	...
	local mobsNearby = mq.TLO.SpawnCount(spawnSearch:format('xtarhater', loot.MobsTooClose))()
	-- options for combat looting or looting disabled
	if deadCount == 0 or ((mobsNearby > 0 or mq.TLO.Me.Combat()) and not loot.CombatLooting) then
		return false
	end
	local corpseList = {}
	for i = 1, math.max(deadCount, limit or 0) do
		local corpse =
			mq.TLO.NearestSpawn(('%d,' .. spawnSearch):format(i, 'npccorpse', loot.CorpseRadius))
		table.insert(corpseList, corpse)
		-- why is there a deity check?
	end
	local didLoot = false
	...
	for i = 1, #corpseList do
		...
		if
			corpseID
			and corpseID > 0
			and not corpseLocked(corpseID)
			and (mq.TLO.Navigation.PathLength('spawn id ' .. tostring(corpseID))() or 100) < 60
		then
...
function loot.lootMyCorpse()
	...
	if mq.TLO.Window('LootWnd').Open() then
		mq.delay(3000, function()
			return mq.TLO.Corpse.Items() and mq.TLO.Corpse.Items() > 0
		end)
		local items = mq.TLO.Corpse.Items() or 0
		if items > 0 then
			for i = 1, items do
				...
				if corpseItem() then
					...
					mq.delay(250, function()
						return not mq.TLO.Corpse.Item(i)() or not mq.TLO.Window('LootWnd').Open()
					end)
					mq.delay(50)
					if not mq.TLO.Window('LootWnd').Open() then
						return
		...
		if containerSize and containerSize > 0 then
			for j = 1, containerSize do
				...
				if itemToSell then
					...
					if sellRule == 'Sell' then
						local sellPrice = bagSlot.Item(j).Value() and bagSlot.Item(j).Value() / 1000
							or 0
						if sellPrice == 0 then
							print(
								logger.logLine(
...
function eventForage()
	...
	while mq.TLO.Cursor() do
		...
		if
			not shouldLootActions[ruleAction]
			or (ruleAction == 'Quest' and currentItemAmount >= ruleAmount)
		then
			if mq.TLO.Cursor.Name() == foragedItem then
				if loot.LootForageSpam then
					print(logger.logLine('Destroying foraged item ' .. foragedItem))
				end
				mq.cmd('/destroy')
				mq.delay(500)
			end
		...
		elseif
			(shouldLootActions[ruleAction] or currentItemAmount < ruleAmount)
			and (not cursorItem.Lore() or currentItemAmount == 0)
			and (mq.TLO.Me.FreeInventory() or (cursorItem.Stackable() and cursorItem.FreeStack()))
		then
			if loot.LootForageSpam then
				print(logger.logLine('Keeping foraged item ' .. foragedItem))
			end
			mq.cmd('/autoinv')
		else
			if loot.LootForageSpam then
				print(logger.logLine('Unable to process item ' .. foragedItem))
			end
			break
		end
		mq.delay(50)
