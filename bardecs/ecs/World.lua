...
function World.New(systemClasses, frequency, disableAutoUpdate)
    local world = setmetatable({
        ...
        _resources = {}, -- Singleton Resource holder shared across all systems
    }, World)
...
function World:AddResource(resource, resName)
    -- Add "resource" to self._resources based on the type of the object itself
   local resourceType = getmetatable(resource).__type
   if resourceType then
      self._resources["resName"] = resource
   else
      error("Resource does not have a __type field in its metatable")
   end
end

function World:Res(resName)
