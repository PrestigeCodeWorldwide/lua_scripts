...
print(
	'\ayHello! Welcome to SheiCamp! Please note that this is my first attempt at creating anything. I hope you enjoy!!')
...
local function tell(delay, gm, aa)
	local z = mq.cmdf('/timed %s /dex %s /multiline ; /stopcast; /timed 1 /alt act %s', delay,
		mq.TLO.Group.Member(gm).Name(), aa)
	return z
end
...
local function the_invis_thing()
	...
	if mq.TLO.Spawn('Group Bard').ID() > 0 then
		...
		print('\ag-->\atINVer: \ay', mq.TLO.Group.Member(inver).Name(), '\at IVUer: \ay',
			mq.TLO.Group.Member(ivuer).Name(), '\ag<--')
...
local npcName = 'a worrisome shade'
local npcID = 5033
	...
	for g = 1, GroupSize, 1 do
		...
		while MemberDistance > 20 do
			...
			if not isMoving then
				mq.cmdf('/multiline ; /dex %s /target a worrisome shade ; /timed 10 /dex %s /nav target', Member, Member)
			end
			mq.delay(1000)
		end
	end
...
local function amIClose()
	...
	if mq.TLO.Target.Distance() > 30 then
		...
		comeToMe()
		DBLinvis()
		print('\apMoving To a worrisome shade')
		mq.cmdf('/dgga /nav spawn id %s', npcID)
		mq.delay(1000)
		WaitOnNav()
	end
end
...
local function SetPull()
	...
	mq.cmdf('/%s pullradius 400', myClass)
	mq.cmdf('/%s pullarch 360', myClass)
	mq.cmdf('/%s zHigh 200', myClass)
	mq.cmdf('/%s zLow 200', myClass)
	mq.cmdf('/%s campradius 20', myClass)
...
local function MoveToCamp()
	...
	DBLinvis()
	mq.delay(1000)
	mq.cmd('/dgga /nav locyx 144.04 -1092.93')
	mq.delay(1000)
	WaitOnNav()
	gmDistanceInstance()
	mq.delay(1000)
	-- Edit this line to match the toons you have CWTN Plugins for. The ShortNames listed ARE the ones you have CWTN for.
	mq.cmd(
		'/noparse /dgge /if (!${Select[${Me.Class.ShortName},BER,ROG,ENC,MNK,MAG,SHD,SHM,CLR,BST,WAR]}) /mac kissassist assist ${Group.MainAssist}')
	print('\apAllowing those Using KissAssist to load up.')
	mq.delay(5000)
	mq.cmd('/dgge /boxr camp ')
	mq.delay(1000)
...
local function DropTask()
	...
	while mq.TLO.Task('Shei Vinitras').ID() and mq.TLO.Me.CombatState() ~= 'COMBAT' do
		mq.delay(1000)
		mq.cmd('/kickp t')
		mq.delay(1000)
		mq.cmd('/yes')
...
--main loop
while Run == true do
	if mq.TLO.Zone.ID() == 843 then
		Missing()
		inME()
		SetCamp = true
		mq.delay(15000)
	else
		if mq.TLO.Zone.ID() == 845 and mq.TLO.Task('Shei Vinitras').ID ~= nil then
			...
			if mq.TLO.Task('Shei Vinitras').ID() ~= nil then
				...
				while mq.TLO.Task('Shei Vinitras').Timer.TotalMinutes() >= 180 do
					mq.delay(1000)
				end
				-- Make sure to change that time here as well!
				if mq.TLO.Task('Shei Vinitras').Timer.TotalMinutes() <= 180 then
					DropTask()
				end
			end
		end
