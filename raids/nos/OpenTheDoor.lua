...
local function on_load()
	...
	if bane and bane.type == 'spell' then
		if mq.TLO.Me.Gem('13')() == spellname then
			return
		else
			StopDPS()
			mq.cmdf('/memspellslot 13 %s', spellname)
			mq.cmdf('/rs Memming %s', spellname)
...
local function main()
	...
	for k, v in ipairs(bane_mob_name) do
		if
			(mq.TLO.SpawnCount(bane_clean_name)() > 3)
			and (mq.TLO.SpawnCount((v .. ' radius 125'):format(v))() > 0)
		then
		...
		if my_class ~= 'BRD' and not bane_ready(bane) then
			return
		end
		StopDPS()
...
while true do
	...
	if
		...
		or mq.TLO.Me.Class.ShortName() == 'RNG'
	then
		on_load()
	end
	mq.doevents()
