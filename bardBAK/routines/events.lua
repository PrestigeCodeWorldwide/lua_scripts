...
function events.eventResist(line, spell_name)
	...
	if target then
		...
		print(
			logger.logLine(
				'\at%s\ax resisted spell \ag%s\ax, resist count = \ay%s\ax',
				target,
				spell_name,
...
function events.eventGear(line, requester, requested)
	...
	elseif slot == 'earrings' then
		...
		mq.cmdf(
			'/gu leftear: %s, rightear: %s',
			leftear.ItemLink('CLICKABLE')(),
			rightear.ItemLink('CLICKABLE')()
		)
	elseif slot == 'rings' then
		local leftfinger = mq.TLO.Me.Inventory('leftfinger')
		local rightfinger = mq.TLO.Me.Inventory('rightfinger')
		mq.cmdf(
			'/gu leftfinger: %s, rightfinger: %s',
			leftfinger.ItemLink('CLICKABLE')(),
	...
	elseif slot == 'wrists' then
		...
		mq.cmdf(
			'/gu leftwrist: %s, rightwrist: %s',
			leftwrist.ItemLink('CLICKABLE')(),
...
local function validateRequester(requester)
	return mq.TLO.Group.Member(requester)()
		or mq.TLO.Raid.Member(requester)()
		or mq.TLO.Spawn('=' .. requester).Guild() == mq.TLO.Me.Guild()
end
...
function events.eventRequest(line, requester, requested)
	...
	if zen.class.isEnabled('SERVEBUFFREQUESTS') and validateRequester(requester) then
		...
		if requested == 'list buffs' then
			local buffList = ''
			for alias, ability in pairs(zen.class.requestAliases) do
		...
		if requested == 'armpet' and state.class == 'mag' then
			table.insert(
				zen.class.requests,
				{ requester = requester, requested = 'armpet', expiration = timer:new(15000) }
			)
			return
		end
		local requestedAbility = zen.class.getAbilityForAlias(requested)
		if requestedAbility then
			local expiration = timer:new(15000)
			table.insert(zen.class.requests, {
				requester = requester,
				requested = requestedAbility,
...
function events.eventTranquil()
	if mq.TLO.Me.CombatState() ~= 'COMBAT' and mq.TLO.Raid.Members() > 0 then
		...
		if zen.class.tranquil:use() then
			mq.cmd('/rs Tranquil Blessings used')
		end
	end
end
