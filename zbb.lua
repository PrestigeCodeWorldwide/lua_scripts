...
-- Bag Contents
local items = {}
local lastItems = {} -- for comparison over time to track new status
...
local function shallow_copy_table(orig)
...
local function create_inventory()
    if (os.difftime(os.time(), start_time)) > INVENTORY_DELAY_SECONDS or #items == 0 then
        start_time = os.time()
        items = {}
        for i = 23, 34, 1 do
...
local function compare_current_inventory() 
...
local function display_bag_options()
    ...
    if ImGui.Checkbox("New", sort_order.new) then
        sort_order.new = true
    else
        sort_order.new = false
    end
    ImGui.SameLine()
    help_marker("Order items with new items appearing first.")
...
local function UpdateInventory()
    lastItems = shallow_copy_table(items)
