...
local ScriptMission = MissionData.heroesareforged

-- Safe point location list we can /nav to for getting unstuck
local SafePoints = {
	...
	for g = 1, GroupSize, 1 do
		...
		while MemberDistance > 20 do
			...
			if not isMoving then
				mq.cmdf(
					"/multiline ; /dex %s /target %s ; /timed 10 /dex %s /nav target",
					Member,
					ScriptMission.npcName,
					Member
				)
			end
			mq.delay(1000)
...
local function gmDistanceInstance()
	...
	for g = 1, GroupSize, 1 do
		...
		while Toon.Distance() > 20 do
			local isMoving = mq.TLO.Group.Member(g).Moving()
			if not isMoving then
				mq.cmdf("/dex %s /nav locyxz 90 -540 0", Member)
			end
			mq.delay(1000)
		end
	end
...
local function amIClose()
	mq.cmd("/boxr pause")
	if mq.TLO.Target.CleanName() ~= ScriptMission.npcName then
		print("\apI Am Not Close Enough To, targeting: " .. ScriptMission.npcName)
		mq.cmdf("/target %s", ScriptMission.npcName)
		mq.delay(1000)
	end

	print("\apMoving To Mission NPC")
	mq.cmd("/nav target")
	WaitOnNav()
end

local function getTask()
	local theZoneImIn = myZone
	local szid = ScriptMission.startingZone.id
	local targdist = mq.TLO.Target

	local currentZone = mq.TLO.Zone.ID()

	printf("GETTING TASK WITH: %d == %d and tar dist: %s", theZoneImIn, szid, targdist)
	if currentZone == ScriptMission.startingZone.id then
		if mq.TLO.Target ~= nil and mq.TLO.Target ~= "NULL" then
			if mq.TLO.Target() ~= nil and mq.TLO.Target() ~= "NULL" then
				if mq.TLO.Target.Distance() <= 30 then
					if mq.TLO.Task(ScriptMission.taskName).ID() == nil then
						...
						mq.delay(2000)
						mq.cmdf("/say %s", ScriptMission.triggerPhrase)
						mq.delay(5000)
					end
				end
			end
...
local function zoneIn()
	...
	local GroupSize = mq.TLO.Group.Members()
	if mq.TLO.Task(ScriptMission.taskName).ID() ~= nil then
		print("\apWaiting For DynamicZone Flagging, Stand By for 30 seconds!")
		mq.delay(30000)
		...
		for g = 1, GroupSize, 1 do
			...
			mq.cmdf("/dex %s /travelto %s", Member, ScriptMission.instanceZone.shortName)
		end
		-- This is to make us the last to zone in
		while mq.TLO.Group.AnyoneMissing() == false do
			mq.delay(2000)
		end
		mq.cmdf("/travelto %s", ScriptMission.instanceZone.shortName)
	end
	--end
end
...
local function DropTask()
	...
	while mq.TLO.Task(ScriptMission.taskName).ID() and mq.TLO.Me.CombatState() ~= "COMBAT" do
		mq.delay(1000)
		mq.cmd("/kickp task")
		mq.delay(1000)
		mq.cmd("/yes")
...
-- heros forge is 862 (herosforge)

local function CheckUnstick()
	-- if i'm not in combat and i'm not moving
	if mq.TLO.Me.Combat() == false and mq.TLO.Me.Moving() == false then
		print("UNSTICKING!")
		-- pause boxes and self
		mq.cmd("/dgga /boxr pause")
		-- pick random direction from SafePoints and nav there for 2 seconds
		local randomSafePoint = SafePoints[math.random(#SafePoints)]
		mq.cmdf("/dgga /nav locyxz %d %d %d", randomSafePoint.y, randomSafePoint.x, randomSafePoint.z)
		mq.delay(2000)
...
--main loop
while Run == true do
	if mq.TLO.Zone.ID() == ScriptMission.startingZone.id then
		print("In Inn")
		Missing()
		--DBLinvis()
		inInstance()
		...
		--print("In else")
		if mq.TLO.Zone.ID() == ScriptMission.instanceZone.id and mq.TLO.Task(ScriptMission.taskName).ID ~= nil then
			--print("In Pallomen")
			while SetCamp == true do
				MoveToCamp()
			end
			if mq.TLO.Task(ScriptMission.taskName).ID() ~= nil then
				print("GOT TASK BY STRING IT WORKS")
				-- Change the Time here based on when you want to exit
				-- Leaving early prevents getting stuck with the task after completion
				while mq.TLO.Task(ScriptMission.taskName).Timer.TotalMinutes() >= 240 do
					CheckUnstick()
					mq.delay(1000)
				end
				-- Make sure to change that time here as well!
				if mq.TLO.Task(ScriptMission.taskName).Timer.TotalMinutes() <= 240 then
					DropTask()
				end
			end
		end
