...
function casting.cast(spell, targetID, interruptCheck)
	...
	if
		state.class == 'brd'
		and spell.MyCastTime <= 500
		and spell.CastType == abilities.Types.Item
	then
		-- do nothing
	...
	if not targetID or targetID == 0 then
		if
			not (
				...
				or spell.TargetType == 'Group v1'
				or spell.TargetType == 'Group v2'
				or spell.TargetType == 'PB AE'
			)
		then
	...
	if target() then
		...
		if spell.BeforeSpell then
			...
			if
				casting.checkReady(spell.BeforeSpellData)
				and casting.checkMana(spell.BeforeSpellData)
			then
		...
		elseif
			spell.CastType == abilities.Types.Skill and mq.TLO.Me.AbilityReady(spell.CastName)()
		then
			-- handle alternate race skill names, slam, dragon punch, tail rake
			mq.cmdf('/doability "%s"', spell.CastName)
			mq.delay(300, function()
				return mq.TLO.Me.AltAbilityReady(spell.CastName)()
			end)
		else
			-- spell, aa, item
...
function casting.memorizeSpell(spell)
	...
	if spell.SpellGem == 0 then
		spell.SpellGem = state.swapGem
	end
	...
	mq.delay(30000, function()
		return not mq.TLO.Window('SpellBookWnd').Open()
	end)
	mq.delay(3000, function()
		return mq.TLO.Me.SpellReady(spell.SpellName)()
	end)
...
function casting.inGlobalCooldown()
	...
	if
		mq.TLO.Me.SpellReady(mq.TLO.Me.Gem(1).Name())()
		or mq.TLO.Me.SpellReady(mq.TLO.Me.Gem(3).Name())()
		or mq.TLO.Me.SpellReady(mq.TLO.Me.Gem(5).Name())()
		or mq.TLO.Me.SpellReady(mq.TLO.Me.Gem(7).Name())()
	then
		return false
	end
	return true
...
function casting.checkReady(spell)
	if spell.CastType == abilities.Types.None then
		return false
	end
	...
	if spell.CastType == abilities.Types.Spell and spell.SpellInBook then
		if spell.SpellName == 'Focused Hail of Arrows' or spell.SpellName == 'Hail of Arrows' then
			if mq.TLO.Me.Gem('Focused Hail of Arrows')() and mq.TLO.Me.Gem('Hail of Arrows')() then
				if not mq.TLO.Me.SpellReady('Focused Hail of Arrows')() then
					return false
				end
				if not mq.TLO.Me.SpellReady('Hail of Arrows')() then
					return false
				end
			end
		end
		if spell.SpellName == 'Mana Flare' or spell.SpellName == 'Mana Recursion' then
			if mq.TLO.Me.Gem('Mana Flare')() and mq.TLO.Me.Gem('Mana Recursion')() then
				if not mq.TLO.Me.SpellReady('Mana Flare')() then
					return false
				end
				if not mq.TLO.Me.SpellReady('Mana Recursion')() then
					return false
				end
			end
		end
