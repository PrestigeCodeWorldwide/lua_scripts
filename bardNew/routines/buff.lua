...
function buff.needsBuff(spell, buffTarget)
	...
	return not buffTarget.Buff(spell.Name)()
		and mq.TLO.Spell(spell.Name).StacksSpawn(buffTarget.ID())()
		and (buffTarget.Distance3D() or 300) < 100
end
...
local function summonItem(buff)
	if
		(
			(buff.summonMinimum or 1) < 0
			or mq.TLO.FindItemCount(buff.SummonID)() < (buff.summonMinimum or 1)
		)
		and not mq.TLO.Me.Moving()
		and (not buff.ReagentID or mq.TLO.FindItemCount(buff.ReagentID)() >= buff.ReagentCount)
	then
...
local function buffCombat(base)
	if not buffCombatTimer:timerExpired() then
		return
	end
		...
		for _, buff in ipairs(base.combatBuffs) do
			if base.isAbilityEnabled(buff.opt) then
				if buff.SummonID then
					...
					if
						not haveBuff(buffName)
						and (not buff.skipifbuff or not mq.TLO.Me.Buff(buff.skipifbuff)())
					then
						abilities.use(buff)
					end
				end
...
local function buffSelf(base)
	...
	for _, buff in ipairs(base.selfBuffs) do
		...
		if buff.SummonID then
			if
				base.isAbilityEnabled(buff.opt)
				and (not buff.nodmz or not constants.DMZ[mq.TLO.Zone.ID()])
			then
			...
			if
				not haveBuff(buffName)
				and not haveBuff(buff.CheckFor)
						...
						state.queuedAction = function()
							if mq.TLO.Me.Casting() then
								...
								mq.delay(100, function()
									return mq.TLO.Me.Buff(buff.RemoveBuff)()
								end)
					...
					elseif buff.RemoveFamiliar then
						...
						state.queuedAction = function()
							if mq.TLO.Me.Casting() or mq.TLO.Pet.ID() == 0 then
								if state.giveUpTimer:timerExpired() then
								...
								if
									...
									and (
										mq.TLO.Pet.Level() == 1
										or mq.TLO.Pet.CleanName():find('familiar')
									)
								then
...
local function buffSingle(base)
	...
	for i = 1, groupSize do
		...
		for _, buff in ipairs(base.singleBuffs) do
			if
				buff.classes[memberClass]
				and mq.TLO.Me.SpellReady(buff.Name)()
				and not member.Buff(buff.Name)()
				and not member.Dead()
				and memberDistance < 100
			then
				member.DoTarget()
				mq.delay(1000, function()
					return mq.TLO.Target.BuffsPopulated()
				...
				if
					mq.TLO.Target.ID() == member.ID()
					and not mq.TLO.Target.Buff(buff.Name)()
					and mq.TLO.Spell(buff.Name).StacksTarget()
				then
					if abilities.use(buff) then
						return true
					end
	...
	for _, aBuff in ipairs(base.groupBuffs) do
		...
		if aBuff.CastType == abilities.Types.Spell then
			...
			if anyoneMissing then
				if abilities.swapAndCast(aBuff, state.swapGem) then
					return true
				end
			end
...
local function buffOOC(base)
	...
	if mq.TLO.SpawnCount('pccorpse radius 25')() > 0 then
		return false
	end
	-- find an actual buff spell that takes time to cast
	if buffAuras(base) then
		return true
	end
	if buffSelf(base) then
		return true
	end
	if buffSingle(base) then
		return true
	end
...
local function buffPet(base)
	if base.isEnabled('BUFFPET') and mq.TLO.Pet.ID() > 0 then
		...
		for _, buff in ipairs(base.petBuffs) do
			...
			if state.subscription ~= 'GOLD' then
				tempName = tempName:gsub(' Rk%..*', '')
			end
			if
				not mq.TLO.Pet.Buff(tempName)()
				and not mq.TLO.Pet.Buff(buff.CheckFor)()
				and mq.TLO.Spell(buff.CheckFor or buff.Name).StacksPet()
				and (not buff.skipifbuff or not mq.TLO.Pet.Buff(buff.skipifbuff)())
			then
				if abilities.use(buff) then
					return
				end
...
function buff.reportBuffs()
	...
	for i = 1, 42 do
		...
		if buffID then
			buffList = buffList .. buffID .. '|'
		end
	end
	for i = 1, 20 do
		local songID = mq.TLO.Me.Song(i).Spell.ID()
		if songID then
			buffList = buffList .. songID .. '|'
		end
	end
	if buffList ~= '' then
		mq.cmdf(
			'/squelch /dga zen /squelch /docommand /$\\{Me.Class.ShortName} bufflist %s %s %s',
			mq.TLO.Me.CleanName(),
...
function buff.reportSick()
	...
	if sickList ~= '' then
		mq.cmdf(
			'/squelch /dga zen /squelch /docommand /$\\{Me.Class.ShortName} sicklist %s %s',
			mq.TLO.Me.CleanName(),
			sickList
		)
	end
...
function buff.buff(base)
	if not state.justZonedTimer:timerExpired() then
		return false
	end
	checkClickiesLoaded(base)
	if buffCombat(base) then
		return true
	end

	if not common.clearToBuff() or not buffOOCTimer:timerExpired() or mq.TLO.Me.Moving() then
		return
	end
	buffOOCTimer:reset()
	...
	if buffOOC(base) or buffPet(base) then
		...
		state.queuedAction = function()
			...
			if targetID ~= 0 and originalTargetID ~= targetID then
				mq.cmd('/squelch /mqt clear')
			else
				mq.cmdf('/squelch /mqt id %s', originalTargetID)
			end
			return action
		end
		return true
