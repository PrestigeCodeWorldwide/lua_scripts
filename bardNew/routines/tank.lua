...
function tank.findMobToTank()
	...
	if
		...
		and state.tankMobID == mq.TLO.Target.ID()
	then
	...
	for id, _ in pairs(state.targets) do
		...
		if mob() then
			...
			if mob.Named() then
				logger.debug(
					logger.flags.routines.tank,
					'Selecting Named mob to tank next (%s)',
					mob.ID()
				)
	...
	if lowesthpid ~= 0 and lowesthp < 100 then
		logger.debug(
			logger.flags.routines.tank,
			'Selecting lowest HP mob to tank next (%s)',
			lowesthpid
		)
	...
	elseif highestlvlid ~= 0 then
		logger.debug(
			logger.flags.routines.tank,
			'Selecting highest level mob to tank next (%s)',
			highestlvlid
		)
	...
	if firstid ~= 0 then
		logger.debug(
			logger.flags.routines.tank,
			'Selecting first available mob to tank next (%s)',
			firstid
		)
...
local function tankMobInRange(tank_spawn)
	...
	if not mob_x or not mob_y then
		return false
	end
...
function tank.stickToMob()
	if
		...
		and mode.currentMode:getName() ~= 'manual'
	then
		mq.cmd('/squelch /stick front loose moveback 10')
		stickTimer:reset()
	end
