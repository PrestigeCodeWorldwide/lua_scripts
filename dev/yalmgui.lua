-- Lootier.  The Lootly UI Companion
-- Version 1.0.6
-- Inspiration goes to Special Ed for his amazing Lootly script
local mq = require("mq")
...
local config_file = "Lootly.ini"
local lootly_config = lip.load(config_dir .. config_file)

local settings_file = lootly_config.Lootly.Lootini
...
Write.prefix = function()
	return "\ax[" .. mq.TLO.Time() .. "] [\agLootier\ax] "
end
Write.loglevel = "info"
...
--- Loads the Lootly loot .ini file into a Lua table
---@param file_path any
...
--- Saves our Lua table back to the Lootly Loot .ini file
---@param file_path any
...
---@param loot_data table A table representing the .ini file used in Lootly
...
--- Load up our data from the Lootly defined loot ini into a format we can use
loadRemoteData(settings_path)

--- Filter our data for displayData
displayData = table.filter(localData, displayFilter)

--- MQ won't run a lua script if it's already running, so I'm being lazy here.
mq.cmd("/lua run yalm")
