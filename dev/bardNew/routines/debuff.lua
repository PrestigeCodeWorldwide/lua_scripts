...
function debuff.shouldUseDebuff(ability)
	...
	elseif ability.opt == 'USESLOWAOE' or ability.opt == 'USESLOW' then
		return mq.TLO.Target()
			...
			and not debuff.SLOW_IMMUNES[mq.TLO.Target.CleanName()]
	elseif ability.opt == 'USESNARE' then
		return mq.TLO.Target()
			and not mq.TLO.Target.Snared()
			and not debuff.SNARE_IMMUNES[mq.TLO.Target.CleanName()]
			and (mq.TLO.Target.PctHPs() or 100) < 40
	else
		return (
			not ability.condition
			and not mq.TLO.Target.Buff(ability.CheckFor or ability.Name)()
			and mq.TLO.Spell(ability.CheckFor or ability.Name).StacksTarget()
		) or (ability.condition and ability.condition())
	end
end

function debuff.findNextDebuff(opt)
	for _, ability in ipairs(zen.class.debuffs) do
		if ability.opt == opt and debuff.shouldUseDebuff(ability) then
			if abilities.use(ability) then
				return true
			end
		end
	end
...
function debuff.castDebuffs()
	if mq.TLO.Target.Type() ~= 'NPC' or not mq.TLO.Target.Aggressive() then
		return
	end
	if zen.class.isEnabled('USEDISPEL') then
		if debuff.findNextDebuff('USEDISPEL') then
			return true
		end
	end
	if zen.class.isEnabled('USEDEBUFFAOE') then
		if debuff.findNextDebuff('USEDEBUFFAOE') then
			return true
		end
	end
	if zen.class.isEnabled('USEDEBUFF') then
		if debuff.findNextDebuff('USEDEBUFF') then
			return true
		end
	end
	if zen.class.isEnabled('USESLOWAOE') then
		if debuff.findNextDebuff('USESLOWAOE') then
			mq.doevents('event_debuffSlowImmune')
			return true
		end
	end
	if zen.class.isEnabled('USESLOW') then
		if debuff.findNextDebuff('USESLOW') then
			mq.doevents('event_debuffSlowImmune')
			return true
		end
	end
	if zen.class.isEnabled('USESNARE') then
...
function debuff.setupEvents()
	if zen.class.OPTS.USESLOW or zen.class.OPTS.USESLOWAOE then
		mq.event(
			'event_debuffSlowImmune',
			'Your target is immune to changes in its attack speed#*#',
			debuff.eventSlowImmune
		)
	end
	if zen.class.OPTS.USESNARE then
		mq.event(
			'event_debuffRunspeedImmune',
			'Your target is immune to changes in its run speed#*#',
			debuff.eventSnareImmune
		)
		mq.event(
			'event_debuffSnareImmune',
