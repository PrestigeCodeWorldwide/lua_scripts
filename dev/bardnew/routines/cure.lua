...
local function cureCounters(toon, buff)
	...
	mq.delay(250, function()
		return mq.TLO.DanNet(toon).QReceived(diseaseCounters)() > 0
			and mq.TLO.DanNet(toon).QReceived(poisonCounters)() > 0
			and mq.TLO.DanNet(toon).QReceived(curseCounters)() > 0
	end)
