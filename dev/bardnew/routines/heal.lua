...
local function getHurt(opts)
	...
	elseif myHP < config.get('HOTHEALPCT') then
		...
		mostHurtDistance = 0
		if myHP < config.get('HEALPCT') then
			numHurt = numHurt + 1
		end
	end
	...
	if tank() and not tank.Dead() then
		...
		if tankHP < config.get('PANICHEALPCT') and distance < 200 then
			return tank.ID(), HEAL_TYPES.PANIC
		end
	end
		...
		for i = 1, groupSize - 1 do
			...
			if not member.Dead() then
				...
				if memberHP < config.get('HOTHEALPCT') and distance < 200 then
					...
					if memberHP < config.get('GROUPHEALPCT') and distance < 80 then
						numHurt = numHurt + 1
					end
					-- work around lazarus Group.MainTank never working, tank just a group member
					if
						tankClasses[member.Class.ShortName()]
						and memberHP < config.get('PANICHEALPCT')
						and distance < 200
					then
	...
	elseif mostHurtPct < config.get('HEALPCT') and mostHurtDistance < 200 then
		return mostHurtID,
			(
				(tankClasses[mostHurtClass] or mostHurtName == config.get('PRIORITYTARGET'))
				and HEAL_TYPES.TANK
			) or HEAL_TYPES.REGULAR
	elseif
		mostHurtPct < config.get('HOTHEALPCT')
		and melees[mostHurtClass]
		and mostHurtDistance < 100
	then
...
local function getHeal(healAbilities, healType, whoToHeal, opts)
	for _, heal in ipairs(healAbilities) do
		if heal[healType] and healEnabled(opts, heal.opt) then
			if
				not heal.tot or (mq.TLO.Me.CombatState() == 'COMBAT' and whoToHeal ~= state.loop.ID)
			then
				if healType == HEAL_TYPES.GROUPHOT then
					if
						mq.TLO.Me.CombatState() == 'COMBAT'
						and groupHOTTimer:timerExpired()
						and not mq.TLO.Me.Song(heal.Name)()
						and heal:isReady()
					then
						return heal
					end
...
function healing.healPetOrSelf(healAbilities, opts)
	...
	if myHP < 60 then
		healing.healSelf(healAbilities, opts)
	end
	if not healEnabled(opts, 'HEALPET') then
		return
	end
	for _, heal in ipairs(healAbilities) do
...
function healing.healSelf(healAbilities, opts)
	if state.loop.PctHPs > config.get('HEALPCT') then
		return
	end
	for _, heal in ipairs(healAbilities) do
		if heal.self then
			...
			if abilities.use(heal) then
				state.queuedAction = function()
					if originalTargetID ~= mq.TLO.Target.ID() then
...
local function doRezFor(rezAbility)
	...
	if
		(config.get('REZGROUP') and mq.TLO.Group.Member(corpseName)())
		or (config.get('REZRAID') and mq.TLO.Raid.Member(corpseName)())
	then
		-- no corpse to rez
		if mq.TLO.Zone.ShortName() ~= 'poknowledge' then
			...
			if newCorpses[corpseName] and not newCorpses[corpseName]:timerExpired() then
				return false
			end
		end
		corpse.DoTarget()
...
function healing.rez(rezAbility)
	if
		(mq.TLO.Zone.ShortName() ~= 'poknowledge' and not rezCheckTimer:timerExpired())
		or not rezAbility
	then
		return
	end
	rezCheckTimer:reset()
	if not config.get('REZINCOMBAT') and mq.TLO.Me.CombatState() == 'COMBAT' then
		return
	end
	if
		rezAbility.CastType == abilities.Types.AA
		and not mq.TLO.Me.AltAbilityReady(rezAbility.CastName)()
	then
		return
	elseif
		rezAbility.CastType == abilities.Types.Spell
		and not mq.TLO.Me.SpellReady(rezAbility.CastName)()
	then
		return
	elseif
		rezAbility.CastType == abilities.Types.Item
		and not mq.TLO.Me.ItemReady(rezAbility.CastName)()
	then
		return
	end
	if mq.TLO.Me.Class.ShortName() == 'NEC' and mq.TLO.FindItemCount('=Essence Emerald')() == 0 then
		return
	end
	if
		rezAbility.CastName == 'Token of Resurrection'
		and (
			mq.TLO.FindItemCount('=Token of Resurrection')() == 0
			or mq.TLO.Me.CombatState() ~= 'COMBAT'
		)
	then
		return
	end
	if reztimer:timerExpired() and mq.TLO.Alert(0)() then
