...
function Ability.use(theAbility, doSwap)
	...
	if not theAbility.timer:timerExpired() then
		return result
	end
...
function Disc:execute()
	...
	if self.overwrite and mq.TLO.Me.ActiveDisc() == self.overwrite then
		mq.cmd('/stopdisc')
	end
...
function Disc:use(overwrite)
	...
	local spell = mq.TLO.Spell(self.Name)
	if (state[self.Name] and not state[self.Name]:timerExpired()) or not self:isReady() then
		return
	end
	if not self:isActive() or not mq.TLO.Me.ActiveDisc.ID() then
		if logger.flags.announce.skill then
...
function Item:isReady(item)
	...
	if state.subscription ~= 'GOLD' and item.Prestige() then
		return false
	end
...
function Item:use()
	...
	if (state[self.Name] and not state[self.Name]:timerExpired()) or not self:isReady(theItem) then
		return false
	end
	if state.class == 'brd' and mq.TLO.Me.Casting() and self.MyCastTime > 500 then
		mq.cmd('/stopcast')
...
function Ability.swapSpell(spell, gem, other_names)
	if not spell or not gem or mq.TLO.Me.Casting() or mq.TLO.Cursor() then
		return
	end
	if mq.TLO.Me.Gem(gem)() == spell.Name then
		return
	end
	if other_names and other_names[mq.TLO.Me.Gem(gem)()] then
		return
	end
