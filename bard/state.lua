...
function state.resetCombatState(debug, caller)
	logger.debug(
		debug,
		'Resetting combatState. pullState before=%s. caller=%s',
		state.pullState,
		caller
	)
	print('reset state')
...
function state.handleCastingState()
	if state.casting then
		...
		if not mq.TLO.Me.Casting() then
			...
			return true
		elseif
			(state.casting.TargetType == 'Single' or state.casting.TargetType == 'Line of Sight')
			and not mq.TLO.Target()
		then
			mq.cmd('/stopcast')
			state.resetCastingState()
			return true
...
function state.handleMoveToCorpseState() end

function state.handleOpenCorpseState() end

function state.handleLootingState() end
