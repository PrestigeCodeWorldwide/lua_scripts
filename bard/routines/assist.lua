...
function assist.getCombatPosition()
	...
	if mq.TLO.Target.LineOfSight() or mq.TLO.Navigation.Active() then
		return false
	end
	--
...
function assist.fsm(reset_timers)
	--, skip_no_los)
	local assistMobID = assist.getAssistSpawnIncludeManual()
	if assistMobID == 0 then
		return false
	end
	if not assist.checkMATargetSwitch(assistMobID) then
		return false
	end
	if not assist.targetAssistSpawn(assistMobID) then
		return false
	end
	assist.setAndAnnounceNewAssistTarget(assistMobID, reset_timers)
