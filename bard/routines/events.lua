...
function events.eventRequest(line, requester, requested)
    ...
    if zen.class.isEnabled('SERVEBUFFREQUESTS') and validateRequester(requester) then
        ...
        if requested == 'armpet' and state.class == 'mag' then
            table.insert(zen.class.requests, {requester=requester, requested='armpet', expiration=timer:new(15000)})
            return
        end
        local requestedAbility = zen.class.getAbilityForAlias(requested)
        if requestedAbility then
            local expiration = timer:new(15000)
            table.insert(zen.class.requests, {requester=requester, requested=requestedAbility, expiration=expiration, tranquil=tranquil, mgb=mgb})
        end
    end
end
...
function events.eventTranquil()
    if mq.TLO.Me.CombatState() ~= 'COMBAT' and mq.TLO.Raid.Members() > 0 then
        ...
        if zen.class.tranquil:use() then mq.cmd('/rs Tranquil Blessings used') end
    end
end
