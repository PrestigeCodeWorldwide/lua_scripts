...
local function checkMobAngle(pull_spawn)
	...
	if pull_arc == 360 or pull_arc == 0 then
		return true
	end
	...
	if not direction_to_mob then
		return false
	end
	-- switching from non-puller mode to puller mode, the camp may not be updated yet
	if not (camp.PullArcLeft and camp.PullArcRight) then
		return false
	end
	logger.debug(
		logger.flags.routines.pull,
		'arcleft: %s, arcright: %s, dirtomob: %s',
		camp.PullArcLeft,
		camp.PullArcRight,
		direction_to_mob
	)
	if camp.PullArcLeft >= camp.PullArcRight then
		if direction_to_mob < camp.PullArcLeft and direction_to_mob > camp.PullArcRight then
			return false
		end
	else
		if direction_to_mob < camp.PullArcLeft or direction_to_mob > camp.PullArcRight then
			return false
		end
	end
	return true
...
local function checkZRadius(pull_spawn)
	...
	if camp.Active then
		if mob_z > camp.Z + config.get('PULLHIGH') or mob_z < camp.Z - config.get('PULLLOW') then
			return false
		end
	else
		if
			mob_z > mq.TLO.Me.Z() + config.get('PULLHIGH')
			or mob_z < mq.TLO.Me.Z() - config.get('PULLLOW')
		then
			return false
		end
	end
...
local function checkMobLevel(pull_spawn)
	if config.get('PULLMINLEVEL') == 0 and config.get('PULLMAXLEVEL') == 0 then
		return true
	end
	local mob_level = pull_spawn.Level()
	if not mob_level then
		return false
	end
...
local function validatePull(pull_spawn, path_len, zone_sn)
	...
	if not mob_id or mob_id == 0 or PULL_TARGET_SKIP[mob_id] or pull_spawn.Type() == 'Corpse' then
		logger.debug(
			logger.flags.routines.pull,
			'Invalid mob ID %s (type=%s, skip=%s)',
			mob_id,
			pull_spawn.Type(),
	...
	if path_len < 0 or path_len > config.get('PULLRADIUS') then
		logger.debug(
			logger.flags.routines.pull,
			'Navigation PathLength %s exceeds PullRadius %s',
			path_len,
			config.get('PULLRADIUS')
		)
		return false
	end
	return checkMobAngle(pull_spawn)
		and checkZRadius(pull_spawn)
		and checkMobLevel(pull_spawn)
		and not config.ignoresContains(zone_sn, pull_spawn.CleanName())
end
...
function pull.checkPullConditions()
	...
	if config.get('GROUPWATCHWHO') == 'self' then
		if
			state.loop.PctEndurance < config.get('MEDENDSTART')
			or state.loop.PctMana < config.get('MEDMANASTART')
		then
			medding = true
			return false
		end
		if
			(
				state.loop.PctEndurance < config.get('MEDENDSTOP')
				or state.loop.PctMana < config.get('MEDMANASTOP')
			) and medding
		then
			return false
		else
		...
		for i = 1, mq.TLO.Group.Members() do
			...
			if member() then
				...
				if member.Dead() then
					return false
				elseif
					healers[member.Class.ShortName()]
					and config.get('GROUPWATCHWHO') == 'healer'
					and pctmana
				then
...
function pull.pullRadarB()
	if not pullRadarTimer:timerExpired() then
		return 0
	end
	pullRadarTimer:reset()
	local pull_radius = config.get('PULLRADIUS')
	if not pull_radius then
		return 0
	end
	...
	local function pullPredicate(spawn)
		...
		if spawn.Distance3D() > pull_radius then
			return false
		end
		local path_len = mq.TLO.Navigation.PathLength(string.format('id %s', spawn.ID()))()
		if not validatePull(spawn, path_len, mq.TLO.Zone.ShortName()) then
			return false
		end
...
function pull.pullRadar()
	...
	if camp.Active then
		...
		logger.debug(
			logger.flags.routines.pull,
			('%s: %s'):format(
				pull_radius_count or 0,
				pull_count_camp:format(camp.X, camp.Y, pull_radius)
			)
		)
	else
		...
		logger.debug(
			logger.flags.routines.pull,
			('%s: %s'):format(pull_radius_count or 0, pull_count:format(pull_radius))
		)
	end
	local shortest_path = pull_radius
	local pull_id = 0
...
local function clearPullVars(caller)
	logger.debug(
		logger.flags.routines.pull,
		'Resetting pull status. beforeState=%s, caller=%s',
		state.pullStatus,
		caller
	)
	state.pullMobID = 0
...
local function pullNavToMob(pull_spawn, announce_pull)
	...
	if announce_pull then
		print(
			logger.logLine('Pulling \at%s\ax (\at%s\ax)', pull_spawn.CleanName(), pull_spawn.ID())
		)
	end
	if helpers.distance(mq.TLO.Me.X(), mq.TLO.Me.Y(), mob_x, mob_y) > 100 then
		logger.debug(
			logger.flags.routines.pull,
			'Moving to pull target (\at%s\ax)',
			state.pullMobID
		)
...
local function pullApproaching(pull_spawn)
	...
	return (config.get('PULLWITH') ~= 'melee' and pull_spawn.LineOfSight() and dist3d < 200)
		or dist3d < 15
		or common.hostileXTargets()
end
...
local function pullEngage(pull_spawn)
	...
	if tot_id > 0 and tot_id ~= mq.TLO.Me.ID() and tot_id ~= mq.TLO.Pet.ID() then
		if targethp and targethp < 99 then
			print(
				logger.logLine(
					'\arPull target already engaged, skipping \ax(\at%s\ax) %s %s %s',
					pullMobID,
					tot_id,
					state.loop.ID,
					targethp
	...
	if mq.TLO.Target.Distance3D() < 35 then
		...
		mq.delay(5000, function()
			return mq.TLO.Me.TargetOfTarget.ID() == state.loop.ID
				or common.hostileXTargets()
				or not mq.TLO.Target()
		end)
	else
		...
		elseif pullWith == 'ranged' then
			...
			if
				ranged_item()
				and ranged_item.Damage() > 0
				and ammo_item()
				and ammo_item.Damage() > 0
			then
				mq.cmd('/squelch /face fast')
				mq.cmd('/autofire on')
				mq.delay(1000)
...
local function pullReturn(noMobs)
	...
	if noMobs and not pullReturnTimer:timerExpired() then
		return
	end
	if helpers.distance(mq.TLO.Me.X(), mq.TLO.Me.Y(), camp.X, camp.Y) < 225 then
		return
	end
...
local function pullMobOnXTarget()
	for i = 1, 20 do
		if mq.TLO.Me.XTarget(i).ID() == state.pullMobID then
			return true
		end
	end
	return false
...
function pull.pullMob()
	...
	if
		anyoneDead()
		or state.loop.PctHPs < 60
		or (mq.TLO.Group.Injured(70)() or 0) > 0
		or constants.DMZ[mq.TLO.Zone.ID()]
	then -- or (state.holdForBuffs and not state.holdForBuffs:timerExpired()) then
		if
			pull_state == constants.pullStates.APPROACHING
			or pull_state == constants.pullStates.ENGAGING
		then
	...
	if
		config.get('LOOTMOBS')
		and mq.TLO.SpawnCount('npccorpse radius ' .. config.get('CAMPRADIUS') .. ' zradius 10')()
			> 0
	then
	...
	if
		not pull_state
		and (state.assistMobID ~= 0 or state.tankMobID ~= 0 or common.hostileXTargets())
	then
	...
	if not pull_state then
		...
		if state.assistMobID ~= 0 or state.tankMobID ~= 0 or common.hostileXTargets() then
			return
		end
	...
	elseif pull_state == constants.pullStates.RETURNING then
		if
			helpers.distance(camp.X, camp.Y, mq.TLO.Me.X(), mq.TLO.Me.Y())
