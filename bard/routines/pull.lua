...
local function validatePull(pull_spawn, path_len, zone_sn)
	...
	if path_len < 0 or path_len > config.get('PULLRADIUS') then
		logger.debug(logger.flags.routines.pull, 'Navigation PathLength %s exceeds PullRadius %s', path_len,
...
function pull.pullRadar()
	...
	if camp.Active then
		...
		logger.debug(logger.flags.routines.pull,
			('%s: %s'):format(pull_radius_count or 0, pull_count_camp:format(camp.X, camp.Y, pull_radius)))
	else
...
local function pullApproaching(pull_spawn)
	...
	return (config.get('PULLWITH') ~= 'melee' and pull_spawn.LineOfSight() and dist3d < 200) or dist3d < 15 or
...
local function pullEngage(pull_spawn)
	...
	if tot_id > 0 and tot_id ~= mq.TLO.Me.ID() and tot_id ~= mq.TLO.Pet.ID() then
		if targethp and targethp < 99 then
			print(logger.logLine('\arPull target already engaged, skipping \ax(\at%s\ax) %s %s %s', pullMobID, tot_id,
				state.loop.ID, targethp))
	...
	if mq.TLO.Target.Distance3D() < 35 then
		...
		mq.cmd('/attack on')
		mq.delay(5000,
			function() return mq.TLO.Me.TargetOfTarget.ID() == state.loop.ID or common.hostileXTargets() or
				not mq.TLO.Target() end)
	else
