...
function commands.commandHandler(...)
	...
	elseif opt == "BURNNOW" then
		print(
			logger.logLine("\arActivating Burns (on demand%s)\ax", state.burn_type and " - " .. state.burn_type or "")
		)
		state.burnNow = true
	...
	elseif opt == "PAUSE" then
		...
		-- Save paused option
		BL.info("Saving paused option")
		config.getOrSetOption(opt, config.get("PAUSED"), tostring(state.paused), "PAUSED")
	...
	elseif opt == "MODE" then
		...
		if new_value then
			new_value = mode.nameFromString(new_value)
		end
