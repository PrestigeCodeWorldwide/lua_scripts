...
function class.initRecoverAbilities(_zen)
	-- Mana Recovery AAs
	class.rallyingsolo =
		common.getAA("Rallying Solo", { mana = true, endurance = true, threshold = 20, combat = false, ooc = true })
...
local function isSongReady(spellId, spellName)
	...
	if
		...
		or (mq.TLO.Spell(spellName).EnduranceCost() > 1000 and state.loop.PctEndurance < state.minEndurance)
	then
		return false
	end
	...
	if not state.loop.Invis and class.doneSinging() and not mq.TLO.Me.Sitting() then
		...
		if mq.TLO.Target.Type() == "NPC" and mq.TLO.Me.CombatState() == "COMBAT" then
			if
				...
				or state.burnActive
				or (class.OPTS.USEEPIC.value == "shm" and mq.TLO.Me.Song("Prophet's Gift of the Ruchu")())
			then
			...
			for _, clicky in ipairs(class.castClickies) do
				...
				elseif
					clicky.Duration == 0
					or (not mq.TLO.Me.Buff(clicky.CheckFor)() and not mq.TLO.Me.Song(clicky.CheckFor)())
				then
